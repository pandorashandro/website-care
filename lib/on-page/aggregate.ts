...
/**
 * Phase 28 — turns every On-Page analyzer's raw output for one analysis
 * into the final, persistence-ready findings. Mirrors
 * lib/architecture/aggregate.ts's proven logic exactly (grouping by
 ...
 * intentionally kept as its own copy rather than a shared generic, for the
 * same documented reason lib/architecture/aggregate.ts gives (CheckKey/
 * FindingCategory differ per engine; a genuine future candidate for
 * lib/category-engine/ extraction, not done here to avoid touching
 * already-accepted code under this phase's own "no scope creep" rule).
 */

...
/**
 * Phase 26, Checkpoint 6 — severity ADJUSTMENT rules, applied once at
 * aggregation time (after every analyzer's raw per-page findings for one
 * check have been grouped into a single candidate finding — see
 * aggregate.ts). Each analyzer already assigns a `baseSeverity` reflecting
 * how bad ONE instance of the problem is in isolation; this function is the
 * one place scope (site-wide vs isolated), spread (affected page count),
 * and confidence adjust that base severity into what actually gets
 * persisted and shown.
 *
 ...
 *
 * `totalAnalyzedPages` is the count of pages this analysis actually
 * evaluated (see run-analysis.ts) — using the crawl's own completed-page
 * count, not effective_page_budget, so a partial crawl's spread percentage
 * is judged against what was actually seen, not against pages that were
 * never reached.
 */

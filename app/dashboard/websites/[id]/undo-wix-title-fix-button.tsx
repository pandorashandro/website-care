...
import { rollbackWixTitleFix, type RollbackWixTitleFixState } from './wix-title-rollback-actions'

const initialState: RollbackWixTitleFixState = null
...
/**
 * Mirrors undo-shopify-title-fix-button.tsx exactly, routed to
 * rollbackWixTitleFix instead. The browser only ever submits websiteId +
 * fixHistoryId — the restore value, resource identity, and drift check are
 * all re-derived and re-verified server-side from the trusted history row.
 */
export default function UndoWixTitleFixButton({
  websiteId,
  fixHistoryId,
  previousValue,
  appliedValue,
  ...
  const [state, formAction, pending] = useActionState(rollbackWixTitleFix, initialState)
    ...
    return (
      <div className="mt-2 border-t border-gray-200 pt-2">
        ...
        <p className="mt-1 text-xs text-gray-600">
          {`Wix title restored to: "${state.restoredTitle}"`}
        </p>
        <p className="mt-1 text-xs text-gray-600">
  ...
  return (
    <div className="mt-2 max-w-sm rounded-md border border-gray-200 bg-white p-3">
      ...
      <p className="mt-3 text-xs text-gray-500">
        If this title has changed in Wix since webioom applied it, the rollback will be stopped rather than

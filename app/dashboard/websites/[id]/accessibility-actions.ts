...
/** Unified webioom engine, Prompt 2 — the ownership-checked entry point into the Accessibility canonical analysis engine, mirroring content-actions.ts's own established pattern exactly. */
...
export type AnalyzeAccessibilityCrawlRunResult = { ok: true; findingsCount: number; healthScore: number } | { ok: false; error: string }

export async function analyzeAccessibilityCrawlRun(websiteId: string, crawlRunId: string): Promise<AnalyzeAccessibilityCrawlRunResult> {
  ...
  const result = await analyzeAccessibility(store, crawlRunId)

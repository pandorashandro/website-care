...
import { rollbackShopifyMetaFix, type RollbackShopifyMetaFixState } from './shopify-meta-rollback-actions'

const initialState: RollbackShopifyMetaFixState = null
...
/** Mirrors undo-shopify-title-fix-button.tsx exactly, routed to rollbackShopifyMetaFix. */
export default function UndoShopifyMetaFixButton({
  websiteId,
  fixHistoryId,
  previousValue,
  appliedValue,
  ...
  const [state, formAction, pending] = useActionState(rollbackShopifyMetaFix, initialState)
    ...
    return (
      <div className="mt-2 border-t border-gray-200 pt-2">
        ...
        <p className="mt-1 text-xs text-gray-600">
          {`Shopify meta description restored to: "${state.restoredValue}"`}
        </p>
        <p className="mt-1 text-xs text-gray-600">
  ...
  return (
    <div className="mt-2 max-w-sm rounded-md border border-gray-200 bg-white p-3">
      ...
      <p className="mt-3 text-xs text-gray-500">
        If this meta description has changed in Shopify since webioom applied it, the rollback will be
        stopped rather than overwriting a newer change.
      </p>

...
/**
 * Phase 26 — an in-memory TechnicalSeoStore used only by tests, mirroring
 * tests/helpers/fake-crawl-store.ts's own precedent: mirrors the real
 * Supabase-backed store's observable behavior (including the
 * (crawl_run_id, analyzer_version) upsert-and-replace semantics) closely
 * enough to exercise lib/technical-seo/run-analysis.ts's orchestration
...
export function createFakeTechnicalSeoStore(seedEvidence: Record<string, CrawlEvidence>) {
  ...
  const store: TechnicalSeoStore & { _analyses: CrawlAnalysisRow[]; _findings: TechnicalFindingRow[]; _findingPages: TechnicalFindingPageRow[] } = {
    ...
    async saveAnalysis(input: SaveAnalysisInput): Promise<CrawlAnalysisRow> {
      ...
      // Replace this analysis's findings wholesale — mirrors the real
      // store's delete-then-insert.
